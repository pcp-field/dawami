...
function loadData(){
 ...
 if(oldGP||oldGS||cycleP||cycleO){
   ...
   d.settings.theme=p.theme||d.settings.theme;d.settings.workHours=Number(p.workHours||d.settings.workHours);d.settings.payRate=Number(p.payRate||d.settings.payRate);d.settings.timezone=p.timezone||d.settings.timezone;d.settings.annualBalance=Number(p.annualBalance||p.leaveBalance||d.settings.annualBalance);d.profile.name=p.name||p.userName||"";
...
function renderReports(){
 ...
 setText("reportHours",Math.round(st.hours)+" ساعة");setText("reportCompare",(st.hours>=old.hours?"أكثر ":"أقل ")+Math.abs(Math.round(st.hours-old.hours))+" ساعة من الشهر الماضي");setText("reportPay",pay.toFixed(3)+" ر.ع");setText("reportNight",st.night+" وردية");setText("reportExtra",st.overtime+" وردية");setText("reportMonthName",monthNames[p.month-1]+" "+p.year);
 $("#reportRows").innerHTML=row("أيام الدوام",st.workDays+" يوم")+row("أيام الراحة",st.offDays+" يوم")+row("أيام الإجازات المسجلة",st.leaveDays+" يوم")+row("الورديات النهارية",st.day)+row("الورديات الليلية",st.night)+row("ساعات العمل",Math.round(st.hours)+" ساعة")+row("الأجر التقديري",pay.toFixed(3)+" ر.ع")
}
...
function bind(){
 ...
 $("#dayType").onchange=function(){var t=this.value;$("#dayLabel").value=eventName({type:t});if(t==="night"){$("#dayStart").value="19:00";$("#dayEnd").value="07:00"}else if(t!=="off"){$("#dayStart").value="07:00";$("#dayEnd").value="19:00"}};
