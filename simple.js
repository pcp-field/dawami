...
function textFor(e){if(!e)return"إجازة";var t=e.type;if(t==="night")return"دوام ليلي";if(t==="off")return"إجازة";if(t==="annual")return"إجازة سنوية";if(t==="leave"||t==="comp"||t==="unpaid"||t==="emergency")return"إجازة مسجلة";if(t==="sick")return"مرضية";if(t==="training")return"تدريب";if(t==="overtime")return"إكسترا دي";return"دوام"}
function classFor(e){if(!e)return"off";if(e.type==="night")return"night";if(e.type==="off")return"off";if(e.type==="overtime")return"extra";if(e.type==="training")return"training";if(e.type==="sick")return"sick";if(["annual","leave","comp","unpaid","emergency"].indexOf(e.type)>=0)return"leave";return"work"}
...
function renderHome(){
 ...
 var workLeft=block?C.workDaysBetween(S,today,block.start):0;$("#daysToLeave").textContent=currentRest?"الحين":workLeft+" يوم";$("#leaveHint").textContent=currentRest?"أنت في الإجازة":"دوام باقي فعليًا";
 $("#leaveStartShort").textContent=exact?new Intl.DateTimeFormat("ar-OM",{timeZone:tz,day:"numeric",month:"short"}).format(new Date(exact.start)):"—";$("#leaveStartFull").textContent=exact?fmtTime(exact.start,{weekday:"long",hour:"numeric",minute:"2-digit"}):"—";
 $("#returnShort").textContent=exact&&exact.next?new Intl.DateTimeFormat("ar-OM",{timeZone:tz,day:"numeric",month:"short"}).format(new Date(exact.next.start)):"—";$("#returnFull").textContent=exact&&exact.next?fmtTime(exact.next.start,{weekday:"long",hour:"numeric",minute:"2-digit"}):"—";
...
function renderCalendar(){
 ...
 for(var i=0;i<42;i++){var k=add(start,i),e=C.primaryEvent(S,k),b=document.createElement("button"),outside=parts(k).month!==p.month;b.className="day "+classFor(e)+(outside?" out":"")+(k===today?" today":"")+(S.overrides[k]!==undefined?" manual":"")+(e.note?" has-note":"");b.innerHTML="<em>"+Number(k.slice(8))+"</em><b>"+textFor(e)+"</b>";b.onclick=(function(x){return function(){showDay(x)}})(k);grid.appendChild(b)}
}
function showDay(k){selectedDay=k;var events=C.eventsForDate(S,k),e=events[0]||{type:"off"},special=["overtime","annual","sick","training"],bits=[];$("#selectedDate").textContent=fmtKey(k,{weekday:"long",day:"numeric",month:"long",year:"numeric"});$("#selectedStatus").textContent=events.map(textFor).join("، ");if(C.isWork(e)&&e.start&&e.end)bits.push("من "+e.start+" إلى "+e.end);if(e.job||e.location)bits.push("الموقع: "+(e.job||e.location));if(e.note)bits.push(e.note);$("#selectedTime").textContent=bits.join(" • ")||"ما عندك إضافة على هذا اليوم";$("#dayType").value=special.indexOf(e.type)>=0?e.type:"overtime";$("#dayStart").value=e.start||data.settings.dayStart;$("#dayEnd").value=e.end||data.settings.dayEnd;$("#dayJob").value=e.job||e.location||"";$("#dayNote").value=e.note||"";$("#dayTimes").hidden=$("#dayType").value==="annual"||$("#dayType").value==="sick";$("#toggleEditDay").textContent=S.overrides[k]!==undefined?"تعديل الإضافة":"أضف على هذا اليوم";$("#editDay").hidden=true;open("dayModal")}
...
function bind(){
 ...
 $("#toggleEditDay").onclick=function(){$("#editDay").hidden=!$("#editDay").hidden};$("#dayType").onchange=function(){var timed=this.value==="overtime"||this.value==="training";$("#dayTimes").hidden=!timed;if(timed&&!$("#dayStart").value){$("#dayStart").value=data.settings.dayStart;$("#dayEnd").value=data.settings.dayEnd}};
 $("#saveDay").onclick=function(){var type=$("#dayType").value,timed=type==="overtime"||type==="training";S.overrides[selectedDay]={type:type,label:textFor({type:type}),start:timed?($("#dayStart").value||data.settings.dayStart):"",end:timed?($("#dayEnd").value||data.settings.dayEnd):"",job:$("#dayJob").value.trim(),note:$("#dayNote").value.trim()};save();close("dayModal");renderAll();toast("تم حفظ اليوم وتلوينه")};$("#resetDay").onclick=function(){delete S.overrides[selectedDay];save();close("dayModal");renderAll();toast("رجع اليوم لجدولك")};
...
function init(){bind();renderAll();setInterval(renderCountdown,1000);if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js?v=4.2").catch(function(){});scheduleNotify();if(!data.onboarded){buildSetup();open("setupModal")}}
init();
})();